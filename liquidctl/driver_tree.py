...
from liquidctl.util import find_all_subclasses
...
def find_devices(pick=None, cls=None, **kwargs):
    ...
    # TODO only search in suitable buses
    num = 0
    for bus_cls in buses:
        for dev in  bus_cls().find_devices(**kwargs):
            if cls and not isinstance(dev, cls):
                continue
            if pick is not None:
                if num == pick:
                    yield dev
